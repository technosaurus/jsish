...
static void lexer_ungetc(int c, Lexer *lex)
{
    ...
    if (lex->ltype == LT_FILE) {
        ...
        lex->cur--;
    } else if (lex->ungot<(sizeof(lex->unch)-2)) {
...
static int iskey(const char *word, Lexer *lex)
{
    ...
    if (!interp->lexkeyTbl->numEntries) {
        ...
        for (i = 0; i < sizeof(keywords) / sizeof(struct st_kw); ++i) {
            hPtr = Jsi_HashEntryCreate(interp->lexkeyTbl, keywords[i].name, &isNew);
            assert(hPtr);
            if (hPtr)
                Jsi_HashValueSet(hPtr, (void*)keywords[i].value);
        }
    }
    hPtr = Jsi_HashEntryFind(interp->lexkeyTbl, word);
    if (hPtr)
...
static char *do_string(Lexer *lex)
{
    ...
    while (!done) {
        if (bufi >= (bsiz-5))
            buf = Jsi_Realloc(buf==unibuf?NULL:buf, bsiz+=BUFSIZ);
        c = lexer_getchar(lex);
        if (c == EOF || c == 0) {
            goto saw_eof;
        }
...
static int _yylex (YYSTYPE *yylvalp, YYLTYPE *yyllocp, Lexer *lex)
{
    ...
    if (isdigit(c)) {
        ...
        Jsi_Number *db = Jsi_Malloc(sizeof(Jsi_Number));
        *db = fval;
        *yylvalp = db;
        return FNUMBER;
    } else if (c == '"' || c == '\'') {
