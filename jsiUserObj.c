...
{
    UserObjReg* ptr = Jsi_Calloc(1, sizeof(*ptr));
    Jsi_HashEntry *hPtr;
    ...
    //ptr->data = data;
    hPtr = Jsi_HashEntryCreate(interp->userdataTbl, udreg->name, &isNew);
    if (hPtr == NULL || !isNew)
        return NULL;
...
{
    Jsi_UserObj *ud = Jsi_Calloc(1,sizeof(Jsi_UserObj));
    ud->interp = interp;
    SIGINIT(ud,USERDATA);
    ud->id = id;
    ud->data = data;
...
int Jsi_UserObjNew(Jsi_Interp *interp, Jsi_UserObjReg* reg, Jsi_Obj *obj, void *data)
{
    ...
    UserObjReg* ureg = Jsi_HashValueGet(hPtr);
    interp->userObjCnt++;
    ...
    uobj->idx = ++ureg->idx;
    int isNew;
    uobj->hPtr = Jsi_HashEntryCreate(ureg->hashPtr, (void*)uobj->idx, &isNew);
...
static void *UserObjGet(Jsi_Interp *interp, Jsi_Obj *obj, Jsi_Hash* id)
{
    ...
    if (obj->ot != JSI_OT_USEROBJ || ud->interp != interp) {
        if (ud->interp != interp)
            Jsi_LogWarn("Jsi_Obj not userdefined type");
        return NULL;
    }
    if (ud->id != id) {
...
void *Jsi_UserObjDataFromVar(Jsi_Interp *interp, char *var) {
