...
//    { "conf",   FilesysConfCmd,     0,  1, "?string|options?",.help="Configure options" , .opts=filesysOptions},
    ...
    { "tell",   FilesysTellCmd,    0,  0, "", .help="Return current position" },
    { "write",  FilesysWriteCmd,   1,  1, "data", .help="Write data to file" },
    { NULL, .help="Commands for accessing File objects" }
};
...
char* Jsi_NormalPath(Jsi_Interp *interp, const char *path, Jsi_DString *dStr) {
    ...
    if (*path == '/')
...
#ifdef __WIN32
        const char *homedir = getenv("USERPROFILE"); /* TODO: windows home dir. */
        if (!homedir)
            homedir = "/";
#else
        /* TODO: use getpwuid_r */
        ...
        if (jsi_pwd) {
            ...
            Jsi_DSAppend(dStr, get_current_dir_name(), NULL);
        }
    } else {
#ifdef __WIN32
full:
        ...
        if (jsi_pwd) {
            ...
            Jsi_DSAppend(dStr, get_current_dir_name(), "/", path, NULL);
        }
    }
    Jsi_DString sStr = {};
...
char * Jsi_GetCwd(Jsi_Interp *interp, Jsi_DString *cwdPtr) {
    ...
    if (cwdFsPtr)
        ...
        Jsi_DSAppend(cwdPtr, get_current_dir_name(), NULL);
