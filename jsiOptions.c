...
static char *typenames[JSI_OPTION_END+1] = {
    "none", "bool", "int", "wide",
    "double", "string", "dstring", "strkey", "strbuf",
    "value", "var", "obj", "array", "func",
    "datetime", "custom", 0
};
...
static Jsi_OptionSpec * GetCachedOptionSpecs(Jsi_Interp *interp, const Jsi_OptionSpec *staticSpecs)
{
    ...
    specCacheTablePtr = Jsi_InterpGetData(interp, "jsi:OptionSpec", NULL);
    ...
    entryPtr = Jsi_HashEntryCreate(specCacheTablePtr, (char *) staticSpecs, &isNew);
    if (isNew) {
    ...
    if (matchPtr == NULL) {
        ...
        for (specPtr = specs; specPtr->type>JSI_OPTION_NONE && specPtr->type < JSI_OPTION_END && specPtr->name; specPtr++) {
            ...
            if (name[0] != '?' || specPtr->type < 0 || specPtr->type >= JSI_OPTION_END) {
                ...
                if (typenames[JSI_OPTION_END] != 0) {
                    fprintf(stderr, "Jsi_OptionTypes out of sync with enum\n");
                    continue;
                }
                Jsi_DSAppend(&dStr, "?", specPtr->name, " <", typenames[specPtr->type], ">? ", NULL);
            }
        }
...
static int
SetOption(Jsi_Interp *interp, Jsi_OptionSpec *specPtr, char *string /*UNUSED*/, void* rec, Jsi_Value *argValue)
{
    char *ptr;
    ...
    case JSI_OPTION_WIDE:
        if (argValue) {
            ...
            if (wcount <= 0) {
                Jsi_LogError("bad value must be positive");
                return JSI_ERROR;
            }
        }
        *((uint *)ptr) = (uint)wcount;
        break;
       ...
       if (argValue)
        {
            if (Jsi_ValueIsNumber(interp, argValue)) {
                ...
                *(Jsi_Wide*)ptr = (Jsi_Wide)num;
                ...
                if (JSI_OK != Jsi_DatetimeParse(interp, scp, "", 0, (Jsi_Wide*)ptr))
                    return JSI_ERROR;
            }
        } else {
            *(Jsi_Number*)ptr = 0;
    ...
    case JSI_OPTION_ARRAY:
        if (!argValue)
            *(Jsi_Value**)ptr = NULL;
            ...
            if (argValue->vt != JSI_VT_OBJECT || !argValue->d.obj->isArray) {
...
int
Jsi_OptionsSet(Jsi_Interp *interp, Jsi_OptionSpec *specs, char *option, void* rec, Jsi_Value *argValue, int flags)
{
    char *record = rec;
    Jsi_OptionSpec *specPtr;
...
GetOption(Jsi_Interp *interp, Jsi_OptionSpec *specPtr, void* record, Jsi_Value *valuePtr, const char *option)
{
    ...
    case JSI_OPTION_CUSTOM:
        if (specPtr->custom) {
            Jsi_OptionCustom* cust = Jsi_OptionCustomBuiltin(specPtr->custom);
            if (cust->formatProc && JSI_OK != ((*cust->formatProc) (interp, specPtr, valuePtr, NULL, record)))
                return JSI_ERROR;
        }
        break;
...
Jsi_Value *
Jsi_OptionsCustomPrint(void* clientData, Jsi_Interp *interp, char *name, void *rec, int offset)
{
    char *record = rec;
    Jsi_Value *valuePtr;
...
Jsi_OptionsProcess(Jsi_Interp *interp, Jsi_OptionSpec *specs,  Jsi_Value *args, void *rec, int flags)
{
    ...
    Jsi_Obj *to;
    if (interp->strict==0 && !(flags&JSI_OPTS_FORCE_STRICT))
        flags |=  JSI_OPTS_IGNORE_EXTRA;
    assert((sizeof(typenames)/sizeof(typenames[0]) == (JSI_OPTION_END+1)));
    specs = GetCachedOptionSpecs(interp, specs);
    ...
    if (args->vt != JSI_VT_OBJECT || args->d.obj->arr) {
    ...
    for (tPtr = Jsi_TreeSearchFirst(to->tree, &search, 0);
        tPtr != NULL; tPtr = Jsi_TreeSearchNext(&search)) {
        
        const char *arg;
        Jsi_Value *optval;
        count++;
        arg = Jsi_TreeKeyGet(tPtr);
        optval = Jsi_TreeValueGet(tPtr);
...
static void DumpOptionSpec(Jsi_Interp *interp, Jsi_Obj *nobj, Jsi_OptionSpec* spec, int addName)
{
    ...
    if (spec->info)
        Jsi_ObjInsert(interp, nobj, "info", Jsi_ValueNewStringKey(interp, spec->info),0);
    Jsi_ObjInsert(interp, nobj, "type", Jsi_ValueNewStringKey(interp, Jsi_OptionTypeStr(spec->type)),0);
    if (spec->init)
    ...
    if (spec->data) {
        if (spec->type == JSI_OPTION_CUSTOM && (spec->custom == Jsi_Opt_SwitchBitset ||
            ...
            Jsi_ObjInsert(interp, nobj, "data", Jsi_ValueNewArray(interp, spec->data, -1), 0);
        } else
...
int Jsi_CDataRegister(Jsi_Interp *interp, const char *name, Jsi_OptionSpec *specs, void *data, int numData, int flags)
{
    ...
    hPtr = Jsi_HashEntryCreate(interp->optionDataHash, name, &isNew);
    if (hPtr == NULL ||
            !(opts = (Jsi_DbMultipleBind*) (isNew ? Jsi_Calloc(1, sizeof(*opts)) : Jsi_HashValueGet(hPtr))))
        return JSI_ERROR;
    opts->opts = specs;
    opts->data = data;
...
static int ValueToVerify(Jsi_Interp *interp, Jsi_OptionSpec* spec, Jsi_Value *inValue, const char *inStr, void *record )
{
    ...
    int (*vfunc)(Jsi_Interp *,Jsi_Value*) = (void*)spec->data;
    if (!vfunc) {
...
static int VerifyToValue(Jsi_Interp *interp, Jsi_OptionSpec* spec, Jsi_Value *outValue, Jsi_DString *dStr, void *record)
{
    ...
    Jsi_Value **s = (Jsi_Value**)(((char*)record) + spec->offset);
    if (*s)
        Jsi_ValueCopy(interp, outValue, *s);
    return JSI_OK;
}
...
void
Jsi_OptionsFree(Jsi_Interp *interp, Jsi_OptionSpec *specs, void *rec, int needFlags)
{
    Jsi_OptionSpec *specPtr;
    char *record = rec;//, *cp;
    for (specPtr = specs; specPtr->type>JSI_OPTION_NONE && specPtr->type < JSI_OPTION_END && specPtr->name; specPtr++) {
...
const char *Jsi_OptionTypeStr(Jsi_OptionTypes typ)
{
...
static Jsi_OptionCustom jsi_OptSwitchEnum = {
    .name="enum", .parseProc=ValueToEnum, .formatProc=EnumToValue, .help="one value from list"
};
...
static Jsi_OptionCustom jsi_OptSwitchBitset = {
    .name="bitset", .parseProc=ValueToBitset, .formatProc=BitsetToValue, .help="An int field accessed a bit at a time"
};
...
static int ValueToSubopt(Jsi_Interp *interp, Jsi_OptionSpec* spec, Jsi_Value *inValue, const char *inStr, void *record )
{
    ...
    Jsi_OptionSpec *subspec = spec->data;
    ...
    if (inValue && Jsi_ValueIsNull(interp, inValue) == 0 &&
        (Jsi_ValueIsObjType(interp, inValue, JSI_OT_OBJECT)==0 || inValue->d.obj->isArray)) {
        Jsi_LogError("expected object");
        return JSI_ERROR;
    }
...
Jsi_OptionCustom* Jsi_OptionCustomBuiltin(Jsi_OptionCustom* cust) {
    if ((int)cust>0 && (int)cust<(sizeof(custOpts)/sizeof(custOpts[0])))
