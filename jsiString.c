    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    if (!spliton || !Jsi_ValueIsType(interp,spliton, JSI_VT_STRING)) {
        Jsi_ValueMakeStringDup(interp,*ret, v);
        return JSI_OK;
    }
    char* split = spliton->d.s.str;

    Jsi_DString sStr;
    Jsi_DSInit(&sStr);
    Jsi_SplitStr(v, &argc, &argv, split, &sStr);
    Jsi_Obj *obj = Jsi_ObjNewType(interp, JSI_OT_ARRAY);
    obj->arr = Jsi_Calloc(argc, sizeof(Jsi_Value*));
    obj->arrCnt = obj->arrMaxSize = argc;
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    if (!len || !Jsi_ValueIsType(interp,len, JSI_VT_NUMBER)) {
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    Jsi_ValueToString(interp, seq);
    char *vseq = seq->d.s.str;
...
static int StringCharCodeAtCmd(Jsi_Interp *interp, Jsi_Value *args, Jsi_Value *_this,
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    Jsi_Value target = VALINIT;
    Jsi_ValueCopy(interp,&target, _this);
    Jsi_ValueToString(interp, &target);
    
    int slen = Jsi_Strlen(target.d.s.str);
    int pos = 0;
    Jsi_Value *vpos;
    ...
    if (pos < 0 || pos >= slen) {
        ...
        Jsi_ValueMakeNumber(interp, *ret, target.d.s.str[pos]);
    }
    Jsi_ValueReset(interp,&target);
    return JSI_OK;
}
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr, int ends)
{
    ...
    if (tchars) {
        tstr = Jsi_ValueToString(interp, tchars);
...
static Jsi_CmdSpec stringCmds[] = {
    { "String",     StringConstructor,      0,  1,  "?str?", JSI_CMD_IS_CONSTRUCTOR, .help="String constructor" },
/*    { "localeCompare",StringLocaleCompareCmd,1, 1, "pos" },*/
    { "charAt",     StringCharAtCmd,        1, 1, "index", .help="Return char at index"  },
    { "charCodeAt", StringCharCodeAtCmd,    1, 1, "index", .help="Return char code at index" },
    { "fromCharCode", StringFromCharCodeCmd,    1, 1, "code", JSI_CMDSPEC_NONTHIS, .help="Return char its code number" },
    { "concat",     StringConcatCmd,        1, -1, "str?,str,...?", .help="Append one or more strings"},
    { "indexOf",    StringIndexOfCmd,       1, 2, "str?,start?", .help="Return index of char" },
    { "lastIndexOf",StringLastIndexOfCmd,   1, 2, "str?,start?", .help="Return index of last char"  },
    { "match",      StringMatchCmd,         1, 1, "pattern",.help="Return array of matches" },
    { "replace",    StringReplaceCmd,       2, 2, "pattern,replace",.help="Return a string after replacement" },
    { "search",     StringSearchCmd,        1, 1, "pattern",.help="Return index of first char matching pattern" },
    { "slice",      StringSliceCmd,         1, 2, "start?,end?", .help="Return section of string" },
    { "split",      StringSplitCmd,         1, 1, "char", .help="Split on char and return Array" },
    { "substr",     StringSubstrCmd,        0, 2, "?start,length?", .help="Return substring" },
    { "substring",  StringSubstrCmd,        0, 2, "?start,length?", .help="Return substring" },
    { "toLocaleLowerCase",StringToLowerCaseCmd,   0, 0, "",.help="" },
    { "toLocaleUpperCase",StringToUpperCaseCmd,   0, 0, "",.help="" },
    { "toLowerCase",StringToLowerCaseCmd,   0, 0, "",.help="Return lower cased string" },
    { "toUpperCase",StringToUpperCaseCmd,   0, 0, "",.help="Return upper cased string" },
    { "toTitle",    StringToTitleCmd,       0, 1, "?chars?",.help="Make first char upper case" },
    { "trim",       StringTrimCmd,          0, 1, "?chars?",.help="Trim chars" },
    { "trimLeft",   StringTrimLeftCmd,      0, 1, "?chars?",.help="Trim chars from left" },
    { "trimRight",  StringTrimRightCmd,     0, 1, "?chars?",.help="Trim chars from right" },
