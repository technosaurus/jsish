...
if (interp->isSafe && Jsi_SafeAccess(interp, fname, writ) != JSI_OK) { \
...
#define MKDBL(nam,val) \
    ...
    Jsi_ValueMakeNumber(interp, nnv, (Jsi_Number)val); \
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    char *filename;
    const char *tp, *template = "/tmp/jsiXXXXXX";

    if (vt && (tp = Jsi_ValueString(interp, vt, NULL))) {
        template = tp;
    }
#ifndef __WIN32
    filename = Jsi_Strdup(template);
...
static int RmdirAll(Jsi_Interp *interp, Jsi_Value *path, int force)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
            ...
            Jsi_ValueMakeString(interp, &tpath, spath);
#ifndef __WIN32
            if (entry->d_type == DT_DIR) {
                int rc = RmdirAll(interp, &tpath, force);
                if (rc != JSI_OK)
                    erc = rc;
            } else 
#endif
            {
                if (Jsi_Remove(interp, &tpath, force) != 0) {
                    if (force)
                        Jsi_LogError("deleting file: %s", GSVal(&tpath));
                    erc = JSI_ERROR;
                }
            }
        }
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    char *linkValue = Jsi_Malloc(MAXPATHLEN + 1);
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    Jsi_ValueMakeString(interp, *ret, Jsi_Strdup(p));
    return JSI_OK;
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    if (lastSlash) {
        Jsi_ValueMakeString(interp, *ret, Jsi_Strdup(lastSlash+1));
    }
    else {
        Jsi_ValueMakeString(interp, *ret, Jsi_Strdup(path));
    }
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    if (*path2 == '/' || *path2 == '~')
        ...
        Jsi_Value tpath = VALINIT;
        Jsi_ValueMakeString(interp, &tpath, Jsi_DSValue(&dStr));
        newname = Jsi_FileRealpath(interp, &tpath, NULL);
        Jsi_DSFree(&dStr);
    }
    if (newname ==  NULL)
        return JSI_ERROR;
...
typedef struct {
    ...
    char recurse;        /* Recurse into directories. */
    char tails;          /* Return only the tails. */
...
pattern,  int isreg, char *path, const char *prefix, GlobData *opts, int deep)
{
    ...
    Jsi_DString tStr = {};
    Jsi_Value rpath = VALINIT;
    Jsi_ValueMakeString(interp, &rpath, path);
    
    if ((n=Jsi_Scandir(interp, &rpath, &namelist, 0, 0)) < 0) {
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    Jsi_Value *dir = NULL;
    Jsi_Value tval = VALINIT;
    Jsi_Value pval = VALINIT;
    GlobData Data;
    memset(&Data, 0, sizeof(Data));
    Data.flags = JSI_FILE_TYPE_FILES;

    if (pat == NULL || Jsi_ValueIsNull(interp, pat)) {
        Jsi_ValueMakeString(interp, &pval, "*");
        pat = &pval;
        ...
        if (pstr && ((cpp=strrchr(pstr, '/')))) {
            ...
            Jsi_ValueMakeStringKey(interp,  &tval, Jsi_DSValue(&dStr));
            dir = &tval;
            
            Jsi_DSSetLength(&dStr, 0);
            Jsi_DSAppend(&dStr, cpp+1, NULL);
            Jsi_ValueMakeStringKey(interp,  &pval, Jsi_DSValue(&dStr));
            pat = &pval;
            Jsi_DSFree(&dStr);
        }
    }
    ...
    if (dir == NULL) {
        Jsi_ValueMakeStringKey(interp, &tval, ".");
        dir = &tval;
    } else {
        ...
        if (dcp && *dcp == '~') {
            ...
            if (dcp) {
                Jsi_ValueMakeString(interp, &tval, dcp);
                dir = &tval;
            }
        }
    }
    if (interp->isSafe && Jsi_SafeAccess(interp, dir, 0) != JSI_OK) {
    ...
    char *pathPtr = Jsi_ValueString(interp, dir, &len);
...
static Jsi_CmdSpec fileCmds[] = {
    { "atime",      FileAtimeCmd,       1,  1, "file",  .help="Return file Jsi_Access time" },
    { "chdir",      FileChdirCmd,       1,  1, "file",  .help="Change current directory" },
    { "chmod",      FileChmodCmd,       2,  2, "file,mode",  .help="Set file permissions" },
    { "copy",       FileCopyCmd,        2,  3, "src,dest?,forcebool?",  .help="Copy a file to destination", .info=FN_copy },
    { "dirname",    FileDirnameCmd,     1,  1, "file",  .help="Return directory path" },
    { "executable", FileExecutableCmd,  1,  1, "file",  .help="Return true if file is executable" },
    { "exists",     FileExistsCmd,      1,  1, "file",  .help="Return status info for file" },
    { "extension",  FileExtensionCmd,   1,  1, "file",  .help="Return file extension" },
    { "join",       FileJoinCmd,        2,  2, "path,path",  .help="Join two file realpaths, or just second if an absolute path" },
    { "isdir",      FileIsdirCmd,       1,  1, "file",  .help="Return true if file is a directory" },
    { "isfile",     FileIsfileCmd,      1,  1, "file",  .help="Return true if file is a normal file" },
    { "glob",       FileGlobCmd,        0,  2, "?pattern?,dir|bool|options??", .help="Return list of files in dir with optional pattern match", .opts=GlobOptions, .info=FN_glob },
    { "link",       FileLinkCmd,        2,  3, "src,dest?,hardbool?",  .help="Link a file", .info=FN_link },
    { "lstat",      FileLstatCmd,       1,  1, "file",  .help="Return status info for file" },
    { "mkdir",      FileMkdirCmd,       1,  1, "file",  .help="Create a directory" },
    { "mtime",      FileMtimeCmd,       1,  1, "file",  .help="Return file modified time" },
    { "owned",      FileOwnedCmd,       1,  1, "file",  .help="Return true if file is owned by user" },
    { "pwd",        FilePwdCmd,         0,  0, "",  .help="Return current directory" },
    { "remove",     FileRemoveCmd,      1,  2, "file?,forcebool?",  .help="Delete a file or direcotry" },
    { "rename",     FileRenameCmd,      2,  3, "src,dest?,forcebool?",  .help="Rename a file, with possible overwrite" },
    { "read",       FileReadCmd,        1,  2, "file?,mode?",  .help="Read a file" },
    { "readable",   FileReadableCmd,    1,  1, "file",  .help="Return true if file is readable" },
    { "readlink",   FileReadlinkCmd,    1,  1, "file",  .help="Read file link destination" },
    { "realpath",   FileRealpathCmd,    1,  1, "file",  .help="Return absolute file name minus .., ./ etc." },
    { "rootname",   FileRootnameCmd,    1,  1, "file",  .help="Return file name minus extension" },
    { "size",       FileSizeCmd,        1,  1, "file",  .help="Return size for file" },
    { "stat",       FileStatCmd,        1,  1, "file",  .help="Return status info for file" },
    { "tail",       FileTailCmd,        1,  1, "file",  .help="Return file name minus dirname" },
    { "tempfile",   FileTempfileCmd,    1,  1, "file",  .help="Create a temp file" },
    { "truncate",   FileTruncateCmd,    2,  2, "file,size",  .help="Truncate file" },
    { "type",       FileTypeCmd,        1,  1, "file",  .help="Return type of file" },
    { "write",      FileWriteCmd,       2,  3, "file,str?,mode?",  .help="Write a file" },
    { "writable",   FileWritableCmd,    1,  1, "file",  .help="Return true if file is writable" },
    { NULL, .help="Commands for accessing and managing files" }
};
