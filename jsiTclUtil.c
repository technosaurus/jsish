...
void Jsi_StackInit(Jsi_Stack *stack)
{
    stack->len = 0;
    stack->maxlen = 0;
    stack->vector = NULL;
...
void Jsi_StackPush(Jsi_Stack *stack, void *element)
{
    ...
    if (neededLen > stack->maxlen) {
        ...
        stack->vector = Jsi_Realloc(stack->vector, sizeof(void *) * stack->maxlen);
    }
...
void Jsi_StackFreeElements(Jsi_Stack *stack, Jsi_DeleteProc *freeProc)
{
    int i;

    for (i = 0; i < stack->len; i++)
        freeProc(stack->vector[i], NULL);
}
...
void Jsi_InterpSetData(Jsi_Interp *interp, const char *key, Jsi_DeleteProc *proc, void *data)
{
    ...
    Jsi_HashEntry *hPtr;
    AssocData *ptr;
    hPtr = Jsi_HashEntryCreate(interp->assocTbl, key, &isNew);
    if (!hPtr)
        return;
    if (isNew) {
...
void Jsi_SetResultFormatted(Jsi_Interp *interp, const char *fmt, ...) {
    ...
    va_end(va);
    printf("<Jsi_SetResultFormatted NOTIMPL>: %s\n", buf);
