...
static Jsi_Value* jsonGenObject(Jsi_Interp *interp, jsmn_parser *p, const char *js, int pos, int *endPos);
static Jsi_Value* jsonGenArray(Jsi_Interp *interp, jsmn_parser *p, const char *js, int pos, int *endPos);

static Jsi_Value*
jsonGen1Value(Jsi_Interp *interp, jsmn_parser *p, const char *js, int i, int *endPos, int incr)
{
    int len;
    const char *t;
    Jsi_Value *v = NULL;
...
static Jsi_Value*
jsonGenObject(Jsi_Interp *interp, jsmn_parser *p, const char *js, int pos, int *endPos)
{
...
static Jsi_Value*
jsonGenArray(Jsi_Interp *interp, jsmn_parser *p, const char *js, int pos, int *endPos)
{
    if (js==NULL || !js[0])
        return NULL;
    ...
    for (i=pos+1, n=0; i<p->toknext && n<tok->size; i++, n++) {
        if (n>= nobj->arrMaxSize) {
            if (jsi_ArraySizer(interp, nobj, n)<=0) {
                break;
            }
        }
...
{
    int i = 0, r, result = JSI_OK, strict = (flags & JSI_JSON_STRICT);
    jsmntok_t *tok;
    Jsi_Value *v;
    const char *err;
    if (js == NULL || js[0] == 0)
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr, int strict)
{
    ...
    js = Jsi_ValueToString(interp, jsv);
...
static Jsi_CmdSpec jsonCmds[] = {
    //{ "JSON",            NULL, 0,  1,  "(str)" },
    { "parse",      JSONParseCmd,       1, 1, "str", .help="Parse JSON and return js" },
    { "parseNS",    JSONScanCmd,        1, 1, "str", .help="Non-strict parse (member names not quoted)"},
    { "check",      JSONCheckCmd,       1, 2, "str,?strict?", .help="Return true if str is JSON" },
    { "stringify",  JSONStringifyCmd,   1, 1, "object", .help="Return JSON from a js object" },
    { "stringifyNS",JSONStringifyNSCmd, 1, 1, "object", .help="Return Non-strict JSON (member names not quoted)" },
