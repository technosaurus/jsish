...
/*
** A JS command for md5.  The argument is the text to be hashed.  The
...
#else
    char buf2[100] = "", *cp = (char*)str;
    unsigned char buf[40];

    if (len<0)
        len = strlen(cp);
    extern char* SHA1(const unsigned char *d, size_t n, unsigned char *md);
    SHA1((unsigned char*)cp, len, buf);
    DigestToBase16(buf, buf2, 20);
    Jsi_ValueMakeStringDup(interp, *ret, buf2);
#endif
    return JSI_OK;
}
                 ...
                 Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    }
    {
        ...
        SHA1((unsigned char*)cp, strlen(cp), buf);
        DigestToBase16(buf, buf2, 20);
    }
    Jsi_Close(in);
    Jsi_ValueMakeStringDup(interp, *ret, buf2);
