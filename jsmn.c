...
#endif
static int jsmn_error(jsmn_parser *parser, int err, char *str) {
    switch (err) {
        case JSMN_ERROR_NOMEM: str = "out of memory";
        case JSMN_ERROR_PART: str = "expected more bytes";
    }
    parser->errStr = str;
    return err;
}
...
static jsmntok_t *jsmn_alloc_token(jsmn_parser *parser, int *indexPtr) {
    ...
    if (parser->toknext >= parser->num_tokens) {
        ...
        if (parser->tokens != parser->static_tokens || parser->static_tokens == NULL)
            parser->tokens = JSMN_REALLOC(parser->tokens, sizeof(jsmntok_t)*parser->num_tokens);
        else {
            parser->tokens = JSMN_MALLOC(sizeof(jsmntok_t)*parser->num_tokens);
...
void jsmn_init(jsmn_parser *parser, jsmntok_t *tokens, 
    ...
    if (tokens == NULL && num_tokens==0)
        num_tokens = 100;
    if (tokens == NULL)
        parser->tokens = malloc(sizeof(jsmntok_t)*num_tokens);
...
const char* jsmn_tokstr(jsmn_parser *parser, const char *js, unsigned int index, int *lenPtr) {
    int len;
    if (index>=parser->toknext)
        return NULL;
