...
static char *CanonicalPath(const char *zRoot, const char *zTail) {
    ...
    zPath = Jsi_Malloc( strlen(zRoot) + strlen(zTail) + 2 );
    if( zPath==0 ) return 0;
    if (zTail[0]) {
...
static ZvfsArchive* ZvfsLookupMount(Jsi_Interp *interp, const char *path) {
    ...
    pEntry=Jsi_HashEntryFirst(local.archiveHash,&zSearch);
    while (pEntry) {
        if ((pArchive = Jsi_HashValueGet(pEntry))) {
            if (!strcmp(Jsi_ValueToString(interp, pArchive->MountPoint), path)) {
                match=Jsi_HashValueGet(pEntry);
                break;
            }
        }
...
static int ZvfsInfoCmd(Jsi_Interp *interp, Jsi_Value *args, Jsi_Value *_this,
                       Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    Jsi_DSInit(&pStr);
    Jsi_DSInit(&mStr);
    if( !local.isInit ) return JSI_ERROR;
    ...
    const char *zMountPoint = Jsi_ValueToString(interp, mount);

    pArchive = ZvfsLookupMount(interp, zMountPoint);
    if (pArchive) {
        if (ret)
            Jsi_ValueCopy(interp, *ret, pArchive->Name);
        else
            rc = JSI_ERROR;
    }
    Jsi_DSFree(&pStr);
...
int Zvfs_Mount( Jsi_Interp *interp, Jsi_Value *archive, Jsi_Value *mount, Jsi_Value *ret)
{
    ...
    if (mount == NULL || Jsi_ValueIsNull(interp, mount)) {
        int n = 0;
        while (n++ < 1000) {
            sprintf(mbuf, "/zvfs%d", n);
            if (!ZvfsLookupMount(interp, mbuf))
                break;
        }
        zMountPoint = mbuf;
    } else {
        zMountPoint = Jsi_ValueToString(interp, mount);
    }
    ...
    */
    ...
    pEntry = Jsi_HashEntryCreate(local.archiveHash, zArchiveName, &isNew);
    if( !isNew ) {
        pArchive = Jsi_HashValueGet(pEntry);
    ...
    */
    ...
    while(1) {
        ...
        */
        int rb = Jsi_Read(chan, zName, lenName);
        if (rb != lenName)
            goto bail;
        isdir=0;
...
addentry:
        ...
        pEntry = Jsi_HashEntryCreate(local.fileHash, zFullPath, &isNew);
        if( isNew ) {
            pZvfs->pNextName = 0;
        } else {
...
static ZvfsFile *ZvfsLookup(Jsi_Interp *interp, Jsi_Value *path) {
    ...
    pEntry = Jsi_HashEntryFind(local.fileHash, zt);
    pFile = pEntry ? Jsi_HashValueGet(pEntry) : 0;
    Jsi_DSFree(&dStr);
...
static int Zvfs_Unmount(Jsi_Interp *interp, Jsi_Value *path) {
    ...
    for(pFile=pArchive->pFiles; pFile; pFile=pNextFile) {
        ...
        if( pFile->pPrevName ) {
            ...
            pEntry = Jsi_HashEntryFind(local.fileHash, pFile->zName);
            if( pEntry==0 ) {
...
static Jsi_Channel Jfz_FSOpenProc (Jsi_Interp *interp, Jsi_Value *path, const char *modes)
{
    ...
    chan = Jsi_Calloc(1,sizeof(Jsi_Chan));
...
static int Jfz_FSWriteProc(Jsi_Channel chan, const char *buf, int size) {
    ZvfsChannelInfo *pInfo = chan->data;
...
writefail:
        ...
        if (status == Z_STREAM_END)
            break;
        if (status != Z_OK) {
            Jsi_LogError("decrypt failed: %s", stream->msg);
            cnt = -1;
            break;
        }
        if (!buf)
...
static Jsi_Channel Jaz_FSOpenProc (Jsi_Interp *interp, Jsi_Value *path, const char *mode)
{
    ...
    if( pInfo->isCompressed ) {
        z_stream *stream = &pInfo->stream;
        pInfo->zBuf = Jsi_Malloc(COMPR_BUF_SIZE);
        stream->avail_in = 2;
        stream->next_in = pInfo->zBuf;
        pInfo->zBuf[0] = 0x78;
        pInfo->zBuf[1] = 0x01;
...
static int Jaz_FSStatProc(Jsi_Interp *interp, Jsi_Value *path, Jsi_StatBuf *buf) {
    ...
    pFile = ZvfsLookup(local.interp, path);
    if( pFile==0 ) {
        return -1;
    }
   ...
   int (*filter)(const Jsi_Dirent *), int (*compar)(const Jsi_Dirent **, const Jsi_Dirent**))
{
    ...
    *namelist = NULL;
    ...
    for(pEntry = Jsi_HashEntryFirst(local.fileHash, &sSearch);
            pEntry && rc == JSI_OK;
        ...
        if (n >= deSpace) {
            deSpace += 50;
            if (de == deS)
                de = Jsi_Calloc(deSpace, sizeof(Jsi_Dirent));
            else
                de = Jsi_Realloc(de, deSpace * sizeof(Jsi_Dirent));
        }
#ifndef __WIN32
...
static int Jaz_FSPathInFilesystemProc (Jsi_Interp *interp, Jsi_Value *path, void* *clientDataPtr) {
    ZvfsFile *zFile;
    if (local.archiveHash->numEntries<=0)
        return 0;
    /*if (ZvfsLookupMount(local.interp, path)==0)
        return 0;*/
    //  TODO: also check this is the archive.
    zFile = ZvfsLookup(local.interp, path);
...
static char * Jaz_FSGetsProc(Jsi_Channel chan, char *s, int size) {
    ...
    *s = 0;
    while (n<(size-1)) {
        if (pInfo->bpos >= pInfo->bsiz) {
            if (pInfo->stream.total_out >= pInfo->pFile->nByte)
                return NULL;
            if (!(pInfo->bsiz = vfsInput(pInfo, pInfo->buf, sizeof(pInfo->buf), &rc))) {
                if (n==0)
                    return NULL;
    ...
    Jsi_Value **ret, Jsi_Func *funcPtr)
{
    ...
    static char *keys[] = {"Name", "Special", "Offset", "Bytes", "BytesCompressed", 0 };
    int i;
    for (i=0; keys[i]; i++)
...
static Jsi_CmdSpec zvfsCmds[] = {
    { "append",     ZvfsAppendCmd,      2, -1, "archive,filelist?,path,filelist,path,...?",  .help="Like 'create()', but appends to an existing archive (with no dup checking)", .info=FN_create },
    { "create",     ZvfsCreateCmd,      2, -1, "archive,filelist?,path,filelist,path,...?",  .help="Create a zip with the given files in prefix path", .info=FN_create },
    { "list",       ZvfsListCmd,        1,  1, "archive",  .help="List files in archive",.info=FN_list },
    { "info",       ZvfsInfoCmd,        0,  1, "?mount?",  .help="Return all zvfs mounted zips, or archive for specified mount", .info=FN_info },
    { "mount",      ZvfsMountCmd,       1,  2, "archive?,mount?",  .help="Mount zip on mount point", .info=FN_mount },
    { "offset",     ZvfsOffsetCmd,      1,  1, "archive",  .help="Return the start offset of zip data", .info=FN_truncate },
    { "stat",       ZvfsStatCmd,        1,  1, "filename",  .help="Return details on file in zvfs mount", .info=FN_stat },
    { "truncate",   ZvfsTruncateCmd,    1,  2, "archive?,noerror?",  .help="Truncate zip data from archive", .info=FN_truncate },
    { "unmount",    ZvfsUnmountCmd,     1,  1, "archive",  .help="Unmount zip" },
...
int Jsi_InitZvfs(Jsi_Interp *interp) {
    if( !local.isInit ) {
        if (Jsi_FSRegister(&Jaz_Filesystem, NULL) != JSI_OK ||
            ...
            return JSI_ERROR;
        local.fileHash = Jsi_HashNew(interp, JSI_KEYS_STRING, NULL);
        local.archiveHash = Jsi_HashNew(interp, JSI_KEYS_STRING, NULL);
        local.isInit = 1;
        local.interp = interp;
    }
