...
HashStringCreate( Jsi_Hash *tablePtr, const void *key, int *newPtr)
{
    ...
    if (newPtr)
        *newPtr = 1;
    size = sizeof(Jsi_HashEntry) + strlen(key) - sizeof(jsi_HashKey) + 1;
    hPtr = Jsi_Malloc(size);
...
HashArray(const void *key, size_t length )
{
    ...
    for (i=0, c=*string++ ; i<length; i++, c=*string++) {
        result += (result<<3) + c;
    }
    return (jsi_Hash)result;
}
...
Jsi_HashEntry *
Jsi_HashEntryCreate(Jsi_Hash *tablePtr, const void *key, int *newPtr)
{
    if (tablePtr->lockProc && (*tablePtr->lockProc)(tablePtr, 1) != JSI_OK)
        return NULL;
...
RebuildTable(Jsi_Hash *tablePtr)
{
    ...
    tablePtr->buckets = Jsi_Calloc(tablePtr->numBuckets, 
...
Jsi_Hash *
Jsi_HashNew(Jsi_Interp *interp, unsigned int keyType, Jsi_DeleteProc freeProc)
{
    Jsi_Hash *tablePtr = Jsi_Calloc(1,sizeof(*tablePtr));
    ...
    for (i = 0; i < tablePtr->numBuckets; i++) {
        ...
        while (hPtr != NULL) {
            ...
            if (tablePtr->freeProc && hPtr->clientData)
                (tablePtr->freeProc)(tablePtr->interp, hPtr->clientData);
            Jsi_Free(hPtr);
            hPtr = nextPtr;
        }
    }
...
Jsi_HashEntry* Jsi_HashSet(Jsi_Hash *tbl, void *key, void *value) {
    Jsi_HashEntry *hPtr;
    int isNew;
    hPtr = Jsi_HashEntryCreate(tbl, key, &isNew);
    if (!hPtr) return hPtr;
...
int Jsi_HashKeysDump(Jsi_Interp *interp, Jsi_Hash *tablePtr, Jsi_Value *ret) {
   char *key;
    int n = 0;
    Jsi_HashEntry *hPtr;
    ...
    if (tablePtr->keyType != JSI_KEYS_STRING && tablePtr->keyType != JSI_KEYS_STRINGPTR)
        return JSI_ERROR;
...
void* Jsi_HashConf(Jsi_Hash *hashPtr, int op, ...) { return NULL; }
